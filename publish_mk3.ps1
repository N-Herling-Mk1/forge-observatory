# =====================================================================
# publish_mk3.ps1  -  the "door 2" button.
# Dev loop stays:  python apps\genre\app\server.py   (local, auto-port)
# When happy:      .\publish_mk3.ps1                 (this file)
#
...
Say "[4b ] ML-stack smoke (torch + bundle load via /api/forge/meta)"
if (Probe "http://127.0.0.1:5601/api/forge/meta" 6 3 "ml stack") { Say "     ok  model stack answers - image carries the full serve path" "Green" }
else { Say "     FAIL - health is up but the ML stack is not (missing deps or bundle);" "Red"
       Say "            docker compose logs genre + check Dockerfile torch layer" "Red"; exit 1 }
...
if ($hasToken) {
    ...
    Say "       4. re-run .\publish_mk3.ps1  (fully hands-off from then on)" "DarkYellow"
}
